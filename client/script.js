...
sayHelloButton.addEventListener("mouseover", changeBW)
...
/*
    ...
    Handle the promise that's returned with a .then, which you should pass a callback function 
    to. Inside the callback function, console.log the response's data (in the intermediate 
        instructions we'll come back to this function and add HTML).
*/ 
...
/*
    ...
    get request to 'http://localhost:3000/repeat/{SOMEPARAM}', but with a string instead of 
    {SOMEPARAM}.  
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get ('http://localhost:3000/repeat/"pikachu"')
...
function p9(query){
    axios.get ('http://localhost:3000/query-test?doYouKnowTheWay&areYouBlue')
    .then (response => {console.log(response.data)})
}
document.querySelector("#query-button").addEventListener("click", p9)
...
/* 
    ...
    the string at the current index (i) and then append the new p element onto the document's 
    body. 
*/
...
//PROBLEM 11
/*
    You are going to add the ability to POST to the server. You'll need to create a small form 
    and write a function that makes a post request. Then you'll attach that function to the 
    submit event on the form. We'll be creating a list of foods. 
...
function createFood(){
    //console.log('clicked')
    let foodInput = document.getElementById('inputFood')
    let body = {
    ...
    axios.post('http://localhost:3000/food', body).then((res) => {
        document.getElementById('foodText').textContent = res.data
        foodInput.value = ''

    })
}
document.getElementById('foodButton').addEventListener('click', function(event) {
